...
document.querySelector('#exec').addEventListener('click', function() {
  ...
  while (후보군.length > hor * ver - mine) { // 값을 하나 뽑는다. 
  ...
  for (var i = 0; i < ver; i += 1) {
    ...
    for (var j = 0; j < hor; j += 1) {
      ...
      td.addEventListener('contextmenu', function (e) { //contextMenu 오른쪽 마우스 
        e.preventDefault();
        if (중단플래그) {
          return;
        }
      ...
      td.addEventListener('click', function (e) {
        ...
        if (dataset[줄][칸] === 코드표.지뢰) { // 지뢰 클릭
          ...
          if (주변지뢰개수 === 0) {
            ...
            주변칸.filter(function (v) { // 배열에서 필터링하는것 배열요소가 X인것
              return !!v;
            }).forEach(function(옆칸) {
